...
## 2026-09-09 - Replaced O(N*M) array search inside shouldInclude with O(M) RegExp lookup
**Learning:** `list.some(c => body.includes(c.toLowerCase()))` creates an O(N*M) bottleneck during policy filtering evaluation, where N is the length of allowed/denied categories and M is the size of the payload body string.
**Action:** By extracting the list to a cached structure and using a pre-compiled `RegExp(..., 'i')`, this allows evaluating entire category checks in a single pass O(M) operation. A helper method `compileCategoryRegexes` ensures consistency between runtime fallback and `setProfile` initialization.
